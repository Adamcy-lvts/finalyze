        ...
        <div v-else-if="isNativeFullscreen"
            ...
            <div class="relative z-10 flex flex-1 overflow-hidden">
                ...
                <Transition enter-active-class="transition-all duration-300 ease-in-out"
                    ...
                    <ChapterEditorRightSidebar
                        v-if="showRightSidebarInFullscreen"
        ...
        <AppLayout v-else
            ...
            <div class="flex h-full w-full overflow-hidden relative">
                ...
                <main
                    ...
                    <!-- Header -->
                    <header
                        class="flex flex-wrap md:flex-nowrap items-start md:items-center justify-between border-b border-border/40 bg-background/60 backdrop-blur-md px-4 py-3 md:py-2 relative z-30 transition-all duration-300 gap-y-2">

                        <!-- Left: Navigation Toggles (Order 1) -->
                        <div class="flex items-center gap-2 flex-shrink-0 order-1">
                            ...
                            <Button variant="ghost" size="icon"
                                ...
                                <PanelLeftClose v-if="!isLeftSidebarCollapsed" class="h-4.5 w-4.5" />
                                <PanelLeftOpen v-else class="h-4.5 w-4.5" />
                            </Button>
                        ...
                        <!-- Right: Mobile Right Sidebar Toggle (Order 2 - Mobile Only) -->
                        <div class="flex md:hidden items-center order-2">
                        ...
                        <!-- Center: Title & Toolbar (Order 3 - New Row on Mobile, Flex-1 on Desktop) -->
                        <div class="order-3 w-full md:w-auto md:flex-1 md:order-none flex flex-col md:flex-row items-start md:items-center gap-3 md:gap-4 md:px-6">
                            
                            <!-- Project Title Area -->
                            <div class="flex items-start md:items-center gap-3 w-full md:w-auto min-w-0">
                                <SafeHtmlText as="h1"
                                    class="text-xl md:text-lg font-bold tracking-tight text-foreground leading-snug cursor-pointer hover:text-primary/80 transition-colors break-words"
                                    :content="props.project.title"
                                    @click="router.visit(route('projects.show', props.project.slug))" />
                                <Badge variant="secondary"
                                    class="shrink-0 h-5 px-2 text-[10px] font-medium rounded-full bg-secondary/50 border border-border/50 mt-1 md:mt-0">
                                    Ch {{ props.chapter.chapter_number }}
                                </Badge>
                            </div>

                            <!-- Toolbar / Actions -->
                            <div class="flex items-center gap-1.5 flex-wrap w-full md:w-auto md:ml-auto">
                                <!-- Desktop Tools Group -->
                                <div class="hidden sm:flex items-center bg-background/50 rounded-full border border-border/50 p-1 backdrop-blur-sm shadow-sm md:mr-2">
                                     <Tooltip>
                                        <TooltipTrigger asChild>
                                            <Button @click="showStatistics = !showStatistics"
                                    ...
                                    <Tooltip>
                                        <TooltipTrigger asChild>
                                            <Button @click="togglePresentationMode"
                                                ...
                                                <span class="hidden xl:inline">{{ showPresentationMode ? 'Edit' : 'Preview' }}</span>
                                            </Button>
                                        </TooltipTrigger>
                                ...
                                <div class="flex items-center gap-2 w-full md:w-auto justify-between md:justify-end mt-2 md:mt-0">
                                    <div class="flex items-center gap-2">
                                        <ExportMenu :project="memoizedProject" :current-chapter="memoizedChapter"
                                            :all-chapters="memoizedAllChapters" :size="isMobile ? 'icon' : 'sm'"
                                            :variant="isMobile ? 'outline' : 'outline'"
                                            :trigger-element="isMobile ? 'icon' : 'button'"
                                            button-class="h-8 w-8 sm:w-auto rounded-full bg-background/50 backdrop-blur-sm"
                                            class="text-zinc-700 dark:text-zinc-300" />

                                        <Button variant="outline" :size="isMobile ? 'icon' : 'sm'"
                                            class="h-8 md:h-8 gap-2 text-zinc-700 dark:text-zinc-300 hover:text-foreground bg-background/50 backdrop-blur-sm rounded-full"
                                            @click="goToBulkAnalysis" title="Open bulk analysis">
                                            <Search class="w-4 h-4" />
                                            <span class="text-xs font-medium hidden md:inline">Analyze</span>
                                        </Button>
                                    </div>
                                    
                                    <div class="flex items-center gap-2">
                                        <Button @click="save(false)" :disabled="isSaving"
                                            :variant="isValid ? 'default' : 'secondary'" :size="isMobile ? 'sm' : 'sm'"
                                            class="h-8 px-3 text-xs shadow-sm gap-2 rounded-full min-w-[32px]">
                                            <Save class="h-3.5 w-3.5" />
                                            <span v-if="!isMobile || isSaving" class="inline">{{ isSaving ? 'Saving' : 'Save' }}</span>
                                        </Button>

                                        <Button @click="markAsComplete" :disabled="isSaving" :size="isMobile ? 'sm' : 'sm'"
                        ...
                        <!-- Right Sidebar Toggle (Order 4 - Desktop Only) -->
                        <div class="hidden md:flex items-center self-center order-last ml-2">
                            <Button variant="ghost" size="icon"
