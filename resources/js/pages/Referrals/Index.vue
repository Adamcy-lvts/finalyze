...
import { Banknote, Users, TrendingUp, Copy, Check, Loader2, AlertCircle, Gift, Star, Building2, ChevronsUpDown } from 'lucide-vue-next'
...
const verifyBankAccount = async () => {
    ...
    try {
        const response = await fetch(route('referrals.verify-bank'), {
            method: 'POST',
            headers: {
...
const setupBankAccount = async () => {
    try {
        const response = await fetch(route('referrals.setup-bank'), {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]')?.getAttribute('content') || '',
...
<template>
    <AppLayout title="Referral Program">
        <div class="container mx-auto px-4 py-8 max-w-6xl">
            <!-- Header -->
            <div class="mb-8">
                <h1 class="text-3xl font-bold tracking-tight">Referral Program</h1>
            ...
            <template v-else-if="!data.bank_setup_complete">
                <Card class="max-w-2xl">
                    <CardHeader>
                        ...
                        <CardDescription>
                            To become a referrer and start earning commissions, you need to setup your bank account first.
