...
// Topic Logic
const allTopicsList = reactive<Topic[]>(props.allTopics.map(topic => ({
    ...topic,
    title: (topic.title || '').replace(/\*\*/g, '').trim()
})));
...
const getFacultyBadgeColor = (faculty?: string) => {
    if (!faculty) return 'bg-zinc-500/10 text-zinc-400 border-zinc-500/20';

    const clean = normalizeText(faculty);
    const colors = [
                ...
                <div>
                    ...
                    <!-- Grid -->
	                    <div v-else class="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                            ...
                            <!-- Category Tag -->
                            <div
                                :class="['absolute top-0 right-0 px-4 py-1.5 text-[10px] uppercase tracking-wider font-semibold rounded-bl-xl border-l border-b', getFacultyBadgeColor(topic.faculty || topic.field_of_study)]">
